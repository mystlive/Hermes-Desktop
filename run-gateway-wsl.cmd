...
wsl.exe -d %HERMES_WSL_DISTRO% -e bash -lc "if [ -n \"$HERMES_WSL_HOME\" ]; then export HERMES_HOME=\"$HERMES_WSL_HOME\"; else export HERMES_HOME=\"${HERMES_HOME:-$HOME/.hermes}\"; fi; export PORT=%HERMES_GATEWAY_PORT%; export PATH=\"$HOME/.local/bin:$PATH\"; if [ -z \"${TERMINAL_CWD:-}\" ]; then if [ -n \"${HERMES_TERMINAL_CWD:-}\" ]; then export TERMINAL_CWD=\"$HERMES_TERMINAL_CWD\"; elif [ -d \"$HERMES_HOME/hermes-builder\" ]; then export TERMINAL_CWD=\"$HERMES_HOME/hermes-builder\"; else export TERMINAL_CWD=\"$HOME\"; fi; fi; cli=\"${HERMES_CLI_PATH:-hermes}\"; pidfile=\"$HERMES_HOME/gateway.pid\"; if [ -f \"$pidfile\" ]; then pid=$(grep -o '[0-9]\+' \"$pidfile\" | head -n 1); if [ -n \"$pid\" ] && kill -0 \"$pid\" 2>/dev/null; then echo \"[Hermes Gateway] Existing gateway already running with PID $pid.\"; exit 0; fi; echo \"[Hermes Gateway] Removing stale gateway.pid ($pid).\"; rm -f \"$pidfile\"; fi; exec \"$cli\" gateway run"
echo.
